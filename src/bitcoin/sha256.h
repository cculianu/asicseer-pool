...
public:
    ...
    CSHA256 &Write(std::span<const uint8_t> data) { return Write(data.data(), data.size()); }
    void Finalize(std::span<uint8_t> hash) { assert(hash.size() == OUTPUT_SIZE); Finalize(hash.data()); }
};
